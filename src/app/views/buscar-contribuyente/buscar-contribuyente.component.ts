...
export class BuscarContribuyenteComponent implements OnInit {
  ...
  validar = ()=>{
    if(this.isEmptyField('codigo') && this.isEmptyField('tipoDocumento')
    && this.isEmptyField('nroDocumento') && this.isEmptyField('nombres')){
      this.buscarError="Debe ingresar al menos un dato!";
      return false;
    }

    if(!this.isEmptyField('tipoDocumento') && this.isEmptyField('nroDocumento')) {
  ...
  isEmptyField(name:string){
    return this.form.controls[name].value==null || this.form.controls[name].value==""; 
  }
  ...
  select(item: any) {
    ...
    this.lista = this.lista.map((tablleItem:any) => { tablleItem.activo = false; return tablleItem;} );
    item.activo = true;
    this.selected=item;
  }
  ...
  editarContribuyente(itemRow: any){
    this.router.navigateByUrl(`/${PATH_REGISTER_TAXPAYER}/${itemRow.contribuyenteNumero}`);
    }
}